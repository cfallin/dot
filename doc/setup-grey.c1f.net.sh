...
docker run --restart=always -d --name letsencrypt -v /home/certs:/etc/nginx/certs:rw --volumes-from nginx-proxy -v /var/run/docker.sock:/var/run/docker.sock:ro thomastweets/docker-letsencrypt-nginx-proxy-companion
...
# texbot
docker run --restart=always -d --name texbot_c1f_net -v /home/cfallin/buildbot/config:/etc/buildbot -e VIRTUAL_HOST=texbot.c1f.net -e LETSENCRYPT_HOST=texbot.c1f.net -e LETSENCRYPT_EMAIL=root@c1f.net -e VIRTUAL_PORT=80 cfallin_buildbot
